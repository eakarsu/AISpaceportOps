...
// an HMAC-SHA256 signature in the X-Spaceport-Signature header, and records the
...
async function fireWebhook(event, payload) {
  ...
  for (const sub of matching) {
    ...
    const headers = {
      'X-Spaceport-Event': event,
      'X-Spaceport-Signature': `sha256=${sig}`,
      'X-Spaceport-Webhook-Id': String(sub.id),
    };
