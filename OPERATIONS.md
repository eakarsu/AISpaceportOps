# Spaceport Ops operations

## Supported operating boundary

The governed path covers range asset telemetry, mission-window validation, independent range-safety approval, execution receipt feedback, and manual recovery. Provider names are typed adapter contracts, not a claim that a live third-party account is connected. Keep outbound execution disabled until each configured telemetry, ERP/WMS/TMS/SCADA/GIS, weather, maintenance, notifications, and range control adapter has passed sandbox contract tests and receipt reconciliation.
...
```sh
node --test backend/governance/tests/*.test.js
bash -n start.sh
./start.sh check
```
